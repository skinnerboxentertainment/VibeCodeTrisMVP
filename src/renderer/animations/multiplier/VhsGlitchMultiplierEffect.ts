 ...
 *     (multiplier: number, decayTimer: number, lastMultiplier: number)
 *
 * Agent note:
...
export class VhsGlitchMultiplierEffect implements IMultiplierEffect {
  ...
  // FIX #1: Add init() method (required by IMultiplierEffect)
  public init(parent: PIXI.Container): void {
    parent.addChild(this.layerContainer);
  }

  // FIX #1: Add destroy() method (required by IMultiplierEffect)
  public destroy(): void {
  ...
  // FIX #2: Correct draw() signature - removed graphics parameter
  draw(
    multiplier: number,
    decayTimer: number,
    ...
    // FIX #3: Use setStrokeStyle() instead of lineStyle()
    ...
    this.distortionGraphics.stroke();
    ...
    const drawOutlineWithOffset = (offsetX: number, color: number, alphaMul: number) => {
      this.outlineGraphics.setStrokeStyle({ width: 1, color: color, alpha: alphaMul * decayAlpha });
      let cursorX = 0;
      for (const char of text) {
